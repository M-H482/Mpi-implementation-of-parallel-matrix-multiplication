...
void divide_AB(int rank, int size, int M, int P, int N, int& Mr, int& Pr, int& Nc,
...
void adjustData(int rank, int size, int Mr, int Pr, int Nc,
    float* A_sub, float* B_sub, float* C_sub)
{
    int row = rank / size, col = rank % size;

    int rp1 = (row + 1) % size, rm1 = (row - 1 + size) % size;
    int cp1 = (col + 1) % size, cm1 = (col - 1 + size) % size;

    MPI_Status sta;
    if (row > size / 2) {
        for (int i = 0; i < size - row; i++) {
            MPI_Sendrecv_replace(A_sub, Mr * Pr, MPI_FLOAT,
                row * size + cp1, 111, row * size + cm1, 111, MPI_COMM_WORLD, &sta);
        }
    } else {
        for (int i = 0; i < row; i++) {
            MPI_Sendrecv_replace(A_sub, Mr * Pr, MPI_FLOAT,
                row * size + cm1, 222, row * size + cp1, 222, MPI_COMM_WORLD, &sta);
        }
    }
    ...
    if (col > size / 2) {
        for (int i = 0; i < size - col; i++) {
            MPI_Sendrecv_replace(B_sub, Pr * Nc, MPI_FLOAT,
                rp1 * size + col, 333, rm1 * size + col, 333, MPI_COMM_WORLD, &sta);
        }
    } else {
        for (int i = 0; i < col; i++) {
            MPI_Sendrecv_replace(B_sub, Pr * Nc, MPI_FLOAT,
                rm1 * size + col, 444, rp1 * size + col, 444, MPI_COMM_WORLD, &sta);
        }
    }
}

void schedule(int rank, int size, int P, int N, int Mr, int Pr, int Nc, float* A_sub, float* B_sub, float* C_sub)
{
    int row = rank / size;
    int col = rank % size;
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (i < size - 1) {
            MPI_Sendrecv_replace(A_sub, Mr * Pr, MPI_FLOAT,
                row * size + cm1, 555, row * size + cp1, 555, MPI_COMM_WORLD, &sta);

            MPI_Sendrecv_replace(B_sub, Pr * Nc, MPI_FLOAT,
                rm1 * size + col, 666, rp1 * size + col, 666, MPI_COMM_WORLD, &sta);
        }
    }
}
...
int main(int argc, char** argv)
{
    ...
    divide_AB(rank, size, M, P, N, Mr, Pr, Nc, A, B, A_sub, B_sub, C_sub);

    double elapsed_time, start, end;
    start = MPI_Wtime();

    adjustData(rank, size, Mr, Pr, Nc, A_sub, B_sub, C_sub);

    schedule(rank, size, P, N, Mr, Pr, Nc, A_sub, B_sub, C_sub);
